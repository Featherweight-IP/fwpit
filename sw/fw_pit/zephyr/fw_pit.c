...
#define FWPIT_BASE DT_INST_REG_ADDR(0)
#define FWPIT_IRW  DT_INST_IRQN(0)
...
int z_clock_driver_init(const struct device *device)
{
	char tmp[16];
	ARG_UNUSED(device);
	sprintf(tmp, "0x%08x", FWPIT_BASE);
	printk("z_clock_driver_init %s\n", tmp);
